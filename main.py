...
# 2. PID 增益系数 (针对 X 轴误差)
#    注意：这里的输出单位是 微秒(us)。
#    Kp: 误差每偏离1像素，PWM脉宽偏移量的“大小”(增益幅度)。
#    注意：Kp 始终取正数！控制方向(正负)已由下方 SERVO_POLARITY 单独表达，
#    不要再把符号塞进 Kp（之前 Kp=-2.0 属于误用，已修正为正增益）。
#    例如：Kp=2.0 表示球偏离1像素，脉宽偏移量改变 2us（方向看极性）。
#    建议起始值: 2.0 ~ 10.0 (取决于舵机灵敏度和距离)
PID_KP = 2.0
...
# 1. 执行器(舵机)方向极性
#    标准 PID 假设：控制输出 u 增大 → 被控量(钢球X)向设定值靠近(负反馈)。
#    但本系统实测“增大脉宽(PWM)会让钢球向 -X(左)移动”，与标准假设相反，
#    因此之前被迫把 Kp 写成负数才能收敛。
#    正确做法：把“方向”与“增益”解耦——Kp 始终取正数，方向由本参数表达：
#      SERVO_POLARITY = +1  表示 增大脉宽使钢球向 +X(右) 移动（标准方向）
#      SERVO_POLARITY = -1  表示 增大脉宽使钢球向 -X(左) 移动（反向安装/接线）
#    当前硬件为反向，故设为 -1；若改接线使“脉宽↑→球右移”，则改为 +1，Kp 仍为正。
#    这样 Kp 始终为正，符号含义清晰，调参不再“负负得正”。
SERVO_POLARITY = -1
...
# 控制方式参考 历程K230pin13引脚控制PWM舵机.py：FPIOA 复用 P13(IO42) 为 PWM0，
# 创建 PWM(0)，freq(50)，用 set_servo_pulse(pulse_us) 将脉宽(us)换算占空比后输出。
#
# 注意：K230 的 PWM 分两个硬件模块，同模块内通道共享频率。PWM0 属于模块0，
# 与板载背光/蜂鸣器等可能同模块。开启显示/媒体后这些外设会持续改写 PWM0
# 的状态，导致舵机“仅初始化瞬间响应、运行期无响应”。解决办法见 set_servo_pulse：
# 每次更新都重新设定 freq + duty 并 enable，把 PWM0 强制拉回舵机所需状态。
# PWM 初始化放在 Display.init / sensor.run 之后，让本程序最后掌控该通道。
